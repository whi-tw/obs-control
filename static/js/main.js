...
var _nosleep = __webpack_require__(/*! nosleep.js */ "./node_modules/nosleep.js/src/index.js");
...
var socket = _socket2.default.connect("http://" + document.domain + ":" + location.port + "/obs");

var noSleep = new _nosleep2.default();
socket.on("current scene", function (msg) {
  $(".card").filter('div[data-scene-name="' + msg.name + '"]').addClass("green");
  $(".card").filter('div[data-scene-name!="' + msg.name + '"]').removeClass("green");
});
socket.on("current webcam", function (webcam) {
  $("#camSelect").val(webcam.name);

  _materializeCss.FormSelect.init($("#camSelect"));
});
$(document).ready(function () {
  $(".card").click(function () {
    var sceneName = $(this).data("scene-name");
    socket.emit("change scene", {
      name: sceneName
    });
  });
  $("#camSelect").change(function () {
    var selectedCam = $(this).children("option:selected").val();
    socket.emit("change webcam", {
      name: selectedCam
    });
  });
